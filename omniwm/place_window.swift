...
// pct <= 0 keeps the window's current size and only centres it.
...
func place(_ win: AXUIElement) {
  ...
  var stable = 0
  repeat {
    var size = pct > 0 ? CGSize(width: w, height: h) : (axSize(win) ?? CGSize(width: w, height: h))
    if pct > 0, let val = AXValueCreate(.cgSize, &size) { AXUIElementSetAttributeValue(win, kAXSizeAttribute as CFString, val) }
